...
class LinkRelevanceModel(BaseRewardModel):
    ...
    async def llm_process_validator_links(self, response):
    ...
    async def process_links(self, results):
        default_val_score_responses = [{} for _ in results]
        ...
        for result in results:
            ...
            random_urls = random.sample(urls, min(3, len(urls)))
            all_links.extend(random_urls)
        ...
        print(f"Fetching {len(unique_links)} unique web links.")
    ...
    async def get_rewards(
        ...
        try:
            val_score_responses_list = await self.process_links(results)
